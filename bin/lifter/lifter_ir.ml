...
module LifterIR : LifterIR = struct
  ...
  let rec string_of_var = function
    ...
    | Add (v, i) ->
        let ch = if i >= 0L then "+" else "" in
        (* for negatives, Int64.to_string produces "-40" *)
  ...
  let rec var_of_string = function
    ...
    | "PSTATE" -> PSTATE
    | s when s.[0] = 'M' ->
        Memory (var_of_string @@ String.sub s 2 (String.length s))
    | s when String.contains s '+' ->
        let sp = String.split_on_char '+' s in
        Add
          ( var_of_string @@ List.hd sp,
            List.rev sp |> List.hd |> Int64.of_string )
    | s when s.[0] = 'R' ->
        Register (int_of_string @@ String.sub s 1 (String.length s))
  ...
  let instruction_eq i1 i2 =
    ...
    match c1 with 0 -> i1.index == i2.index | _ -> false
