...
module SpecParse : SpecParse = struct
  ...
  let kind = take_while1 (fun c -> is_space c |> not) >>| function
    | "==" -> Kind.Equal
    | "!=" -> Kind.Distinct
    | "~" | "!" -> Kind.Not
    | "&&" -> Kind.And
  ...
  let spec_body = fix (fun (recurse : SpecLang.spec_body t) ->
    let pre = string "pre" *> with_parens var >>| (fun a b -> SpecLang.Pre (b, a)) in
    let post = string "post" *> with_parens var >>| (fun a b -> SpecLang.Pre (b, a)) in
    let predicate = (with_parens (pre <|> post)) <*> (take_while is_word) in

    let const = take_while1 is_digit >>| int_of_string >>| (fun i -> SpecLang.Const i) in
    ...
    let term = with_parens term_inner >>| (fun (k, subs) -> SpecLang.Term (k, subs)) in
  ...
  let parse s = parse_string ~consume:All all_specs s |>
    function
    | Ok a -> List.fold_left (fun acc (k,v) -> SpecLang.M.add k v acc) SpecLang.M.empty a
    | Error s -> failwith s
end
