...
module Gtirb = struct
    ...
    let parse (ir: p_ir) =
        ...
        let intervals   = flatmap (fun (s: p_section)  -> s.byte_intervals) sections in
        let blocks      = flatmap (fun (i: p_interval) -> i.blocks) intervals in

        (* symbolic_expressions is originally a map (int * p_symexprs option) *)
        let unmap t     = (List.filter_map
                             (fun (i: int * p_symexprs option) -> match i with (_, a) -> a)
                           t) in
        let exprs       = flatmap (fun (i: p_interval) -> (unmap i.symbolic_expressions)) intervals in
        ignore exprs; (* we don't need it yet, but it's a hassle to extract, so the code is staying in *)
    ...
    let symbol_by_name (ss: p_symbol list) (name: string): p_symbol =
        let matches (s: p_symbol) = if (String.equal s.name name)
                                    then Some s
                                    else None in
    ...
    let expect_referent_uuid (s: p_symbol): bytes =
        ...
        | _ -> failwith (Printf.sprintf "Symbol %s does not refer to a block!" s.name)
    ...
    let block_by_uuid (bs: p_block list) (uuid: bytes): p_block =
        let matches (b: p_block) = match b.value with
        ...
        | Some b -> b
        | _ -> failwith (Printf.sprintf "No block with uuid %s" (Bytes.to_string uuid))
    ...
    let codeblock_by_uuid (bs: p_block list) (uuid: bytes): p_code =
        ...
        | `Code (cb: p_code) -> cb
        | _ -> failwith (Printf.sprintf "Referenced block %s is not a code block!" (Bytes.to_string uuid))


    (* CFG helpers *)
    let traverse_cfg_uuids (cfg: p_cfg) (u: bytes): bytes list =
        let targets (e: p_cfgedge) = if (Bytes.equal e.source_uuid u) then Some e.target_uuid else None in
        List.filter_map (targets) cfg.edges

    let all_func_codeblocks (bs: p_block list)
                            (cfg: p_cfg)
                            (u: bytes) = List.map (codeblock_by_uuid bs) (traverse_cfg_uuids cfg u)
