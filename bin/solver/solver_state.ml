...
module type SolverState = sig
  ...
  type state_function = (string -> Term.term -> Term.term option)

  val initialise : Cvc5.Solver.solver -> Sort.sort -> string list -> state
  val link_aliases : (string * string) list -> state -> state

  val apply : ?rely:bool -> Cvc5.Solver.solver -> Sort.sort -> state_function -> state -> state
end
...
module SolverState : SolverState = struct
  ...
  let initialise slv srt (names : string list) : state =
    (S.empty, List.fold_left (fun acc n -> S.add n (Cvc5.Solver.declare_sygus_var slv n srt) acc) S.empty names)

  let link_aliases (pairs : (string * string) list) (state : state) =
    (List.fold_left (fun acc (n1, n2) -> S.add n1 n2 acc) (fst state) pairs, snd state)

  let nondeterminism_counter = ref 0
  let fresh_nondeterminism slv srt =
    incr nondeterminism_counter;
    Cvc5.Solver.declare_sygus_var slv ("???_" ^ (string_of_int !nondeterminism_counter)) srt

  let apply ?(rely=false) slv srt func (state : state) =
    let map = S.mapi (fun (k : string) (v : Term.term) ->
      if ((not rely) && String.starts_with ~prefix:"R" k) then v
      else match (func k v) with
