...
module type SolverSpec = sig
  val translate_fn :
    TermManager.tm -> Spec.Lang.spec -> SolverState.spec_predicates -> SolverState.state_function

  val translate_cn :
    TermManager.tm -> Spec.Lang.spec -> SolverState.spec_predicates -> SolverState.state_constraints

  val generate_pres : Spec.Lang.spec * Spec.Lang.spec -> (string * string * bool) list list
end
...
module SolverSpec : SolverSpec = struct
  let rec ast_convert tm (b : Spec.Lang.body) (s : SolverState.state)
    ...
    | Pre (pred, name) -> Some (Term.mk_term tm Kind.Apply_uf @@ Array.of_list [SolverState.find_predicate p pred; input_term name])
    | Post (pred, name) -> Some (Term.mk_term tm Kind.Apply_uf @@ Array.of_list [SolverState.find_predicate p pred; input_term name])
    | Nondeterminism -> None

  let translate_fn tm (spec : Spec.Lang.spec) p : SolverState.state_function =
    let functions =
      List.filter_map
        (function
    ...
    fun state s ->
      let body =
        ...
        |> Option.map snd
      in
      Option.bind body (fun b -> ast_convert tm b state None p)

  let translate_cn tm (spec : Spec.Lang.spec) p : SolverState.state_constraints =
    let constraints =
      List.filter_map
        (function
  ...
  let generate_pres spec =
    ...
    let pred_uses = (fst spec @ snd spec) |>
      List.map (function Spec.Lang.Constraint b | Spec.Lang.Function (_, b) -> b) |>
      List.map collect_preds |>
      List.flatten
    in

    let rec combine = function
      | 0 -> [[]]
      | n -> let c = combine (n - 1) in List.map (List.cons true) c @ List.map (List.cons false) c
    in
    let all = List.length pred_uses |> combine in
