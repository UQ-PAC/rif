...
let llvm_initialize_all_target_infos =
  foreign "LLVMInitializeAArch64TargetInfo" (void @-> returning void)

let llvm_initialize_all_target_mcs =
  foreign "LLVMInitializeAArch64TargetMC" (void @-> returning void)

let llvm_initialize_all_disassemblers =
  foreign "LLVMInitializeAArch64Disassembler" (void @-> returning void)

let llvm_create_disasm =
  foreign "LLVMCreateDisasm"
    (string @-> ptr void @-> int @-> ptr void @-> ptr void
    @-> returning (ptr void))

let llvm_disasm_instruction =
  foreign "LLVMDisasmInstruction"
    (ptr void @-> ptr char @-> size_t @-> size_t @-> ptr char @-> size_t
   @-> returning int)
...
let disassembler : unit ptr =
  ...
  if dc == null then failwith ("Error creating disassembler for " ^ triple);
  dc
...
let hexstring_to_opcode (s : string) = byte_list (int_of_string s)
...
let assembly_of_bytelist (opcode : char list) : string =
  ...
  let outb =
    llvm_disasm_instruction disassembler (CArray.start oc)
      (Unsigned.Size_t.of_int 4) (Unsigned.Size_t.of_int 0) (CArray.start out)
      (Unsigned.Size_t.of_int array_len)
  in
  if outb == 0 then raise (Failure "Error disassembling instruction.")
  else
    let takeWhile (p : 'a -> bool) (ar : 'a list) : 'a list =
      let rec _take (c : 'a list) (a : 'a list) =
        match a with
    ...
    let strout =
      String.concat ""
        (List.map
           ...
           (takeWhile (fun c -> c != '\000') (CArray.to_list out)))
    in
    String.trim @@ String.map (fun f -> if f == '\t' then ' ' else f) strout
...
let assembly_of_int_opt (opcode : int) : string option =
  match assembly_of_int opcode with exception Failure _ -> None | x -> Some x
